...
export default function LiveTicker() {
  ...
  return (
    <div className="border-b border-blue-400/20 bg-gradient-to-r from-slate-950 via-blue-950/70 to-slate-950">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-2.5 sm:px-6">
        <span className="hidden shrink-0 items-center gap-2 rounded-full bg-green-500/15 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-green-300 sm:inline-flex">
          ...
          <div className="ticker-track flex w-max gap-10">
            {items.map((item, i) => (
              <div key={`${item.text}-${i}`} className="flex shrink-0 items-center gap-2 text-sm text-slate-300">
                ...
                <span className="whitespace-nowrap text-xs text-blue-300/70">· {item.time}</span>
              </div>
            ))}
          </div>
        </div>
